...
export default class Frak
{
  ...
  constructor(settings = {})
  {
    ...
    this._settings =
    {
      // Frak specific setting
      throwErrorOnFailedStatus: false,
      resolveJsonResponse: true,
      request:
      {
          headers: {},
    ...
    // If user passed in a settings object then allow overrides
  if (typeof settings.throwErrorOnFailedStatus === 'boolean') {
    this._settings.throwErrorOnFailedStatus = settings.throwErrorOnFailedStatus;
  }

  if (typeof settings.resolveJsonResponse === 'boolean') {
      this._settings.resolveJsonResponse = settings.resloveJsonResponse;
  }

  if (typeof settings.request === 'object') {
      this._settings.request = settings.request;
  }
  ...
  */
  ...
  _initializeRequest(method, request, body)
  {
    ...
    let _request = this._settings.request;
    let _headers = _request.headers;

    // Populate the headers from the defaults per the http method.
    let headersArray = DEFAULT_REQUEST_HEADERS[method];
    headersArray.forEach((element) =>
    {
      _headers = {..._headers, element};
    });

    // Merge the default request object with the passed in request object.
    _request = {..._request, request};

    // If body is present then transform (if needed) and add it as a property to the request.
    if (body) {
        _request.body = (typeof body) === 'object' ? JSON.stringify(body) : body;
    }
  ...
  _call(url, request)
  {
    ...
    async function doFetch(url, request)
    {
      ...
      // Resolve to Json if content-type is application/json?
      let resolveJsonResponse = settings.resolveJsonResponse;
      ...
      try {
          ...
          if (resolveJsonResponse && response.headers)
          {
