...
export default class Frak
{
     ...
     * @param {string} [mode] Mode - defaults to 'cors', valid values are 'no-cors, cors, *same-origin'
     */
    constructor(throwErrorOnFailedStatus = false, mode = 'nocors')
    {
    ...
    _initializeRequest(method, request, body)
    {
        ...
        if (!request) {
            request = {headers: {}, mode: this._mode};
        }
